...
def load_table(uploaded_file):
    ...
    # Try different CSV parsing options with comprehensive encoding support
    encoding_options = [
        {},  # Let pandas auto-detect
        {"encoding": "utf-8"},
        {"encoding": "utf-8-sig"},  # UTF-8 with BOM
        {"encoding": "latin1"},     # Common fallback encoding
        {"encoding": "iso-8859-1"}, # Alternative name for latin1
        {"encoding": "cp1252"},     # Windows encoding
        {"sep": ";"},               # Different separator
        {"sep": ";", "encoding": "latin1"},  # European CSV with latin1
    ...
    # If stream-based reading fails, try reading raw bytes
    try:
        uploaded_file.seek(0)
        raw = uploaded_file.getvalue() if hasattr(uploaded_file, "getvalue") else uploaded_file.read()
